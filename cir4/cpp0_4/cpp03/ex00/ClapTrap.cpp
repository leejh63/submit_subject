...
void ClapTrap::log_print( const std::string& msg ) const {
	if(skip_p) { return; }
	std::cout << "\n[" << ClapTrap::g_class() << "] " << msg << "\n";
}

ClapTrap::ClapTrap( void )
: _name("unknown"), _h_point(10), _e_point(10), _damage(0)
	{
	log_print( "Default Constructor" );
}

ClapTrap::ClapTrap( const std::string& name )
: _name(name), _h_point(10), _e_point(10), _damage(0)
	{
	log_print( "Name Constructor" );
}
...
:	_name(copy._name), _h_point(copy._h_point),
	...
	{
	log_print( "Copy Constructor" );
}

ClapTrap& ClapTrap::operator=( const ClapTrap& copy ) {
	log_print( "Assignment Operator" );
	if (this != &copy) {
...
ClapTrap::~ClapTrap( void ) {
	log_print( "Destructor" );
}

void ClapTrap::attack( const std::string& target ) {
	log_print( "attack(target)" );

	if (this->_h_point == 0) { log_print( "Already Dead" ); return; }

	if (this->_e_point == 0) { log_print( "Not enough Energy" ); return; }
