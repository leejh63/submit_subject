...
void	draw_column(t_ray *r, t_param *p, int tex_x, int pix_x)
{
	...
	/*/
	view = WIN_H + (int)(WIN_H * (sin(p->v.verti) - cos(p->v.verti)));
	i = 0;
	while (i < WIN_H)
	{
		//hit_y = (0.5 - jump) + (r->dist * (i * 2 - view) / (WIN_H));
		hit_y = 0.5 + (r->dist * (i * 2 - view) / (WIN_H));
		if (hit_y < 0.0)
			color = p->map.c;
		else if (hit_y > 1)
			color = p->map.f;
		else
			color = export_color(r->texture, tex_x, hit_y * IMG_SIZE);
