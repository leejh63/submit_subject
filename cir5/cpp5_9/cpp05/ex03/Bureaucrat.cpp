...
Bureaucrat::GradeTooHighException::GradeTooHighException( void ) throw() {
	util_::log("GradeTooHighException Constructor");
}

const char* Bureaucrat::GradeTooHighException::what( void ) const throw() {
	util_::log("GradeTooHighException what");
	return "[B]Grade is too HIGH!";
}

Bureaucrat::GradeTooLowException::GradeTooLowException( void ) throw() {
	util_::log("GradeTooLowException Constructor");
}

const char* Bureaucrat::GradeTooLowException::what( void ) const throw() {
	util_::log("GradeTooLowException what");
	return "[B]Grade is too LOW!"; 
}
...
void Bureaucrat::signForm( AForm& paper) const {
	util_::log("signForm");
	std::string paper_name = paper.getName();
	std::string paper_class = paper.getClass();
	try{
		paper.beSigned(*this);
		std::cout << this->name_ << " signed " << paper_name << "\n";
	}
	catch(std::exception &e) {
		std::cerr << this->name_ << " couldn’t sign " 
				  << paper_name << " because "
				  << " [" << paper_class << "]"
				  << e.what() << std::endl;
		std::cout << "\n";
...
void Bureaucrat::executeForm( const AForm& paper ) const {
	util_::log("executeForm");
	std::string paper_name = paper.getName();
	std::string paper_class = paper.getClass();
	try{
		paper.beExecuted(*this);
		std::cout << this->name_ << " executed " << paper_name << "\n";
	}
	catch(std::exception &e) {
		std::cerr << this->name_ << " couldn’t execute " 
				  << paper_name << " because "
				  << " [" << paper_class << "]"
				  << e.what() << std::endl;
	}
