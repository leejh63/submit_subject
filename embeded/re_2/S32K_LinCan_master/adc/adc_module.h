...
/*
 * portable 모듈에 전달되는 보드 전용 ADC 설정 구조체다.
 ...
 * 이 snapshot은 raw 값과 semantic zone, latch 상태,
 * 오류 플래그를 하나의 구조체에 묶어 쉽게 게시할 수 있게 한다.
 */
typedef struct
{
    uint16_t raw_value;
...
/*
 * 하나의 펌웨어 이미지가 가지는 전체 ADC 모듈 상태다.
 * 설정과 최신 snapshot,
 * 그리고 주기 ADC task가 사용하는 sample timestamp를 함께 보관한다.
 */
typedef struct
{
    uint8_t     initialized;
