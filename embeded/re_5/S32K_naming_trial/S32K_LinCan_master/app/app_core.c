...
InfraStatus AppCore_Init(AppCore *app)
{
    ...
    app->lin_last_reported_latch = 0xFFU;
...
void AppCore_TaskCan(AppCore *app, uint32_t now_ms)
{
    ...
    app->had_can_activity = activity;
    if (activity != 0U)
    {
        app->can_active_tick_count++;
    }
}
...
void AppCore_TaskRender(AppCore *app, uint32_t now_ms)
{
    ...
    can_text = (app->had_can_activity != 0U) ? "ok" : "idle";
    ...
    (void)snprintf(task_text,
                   ...
                   can_text,
                   (unsigned long)app->can_active_tick_count,
                   app->lin_link_text,
                   uart_text,
