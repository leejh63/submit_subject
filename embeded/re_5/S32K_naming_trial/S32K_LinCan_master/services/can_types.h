// CAN 프로토콜 공통 상수와 자료구조를 모아둔 헤더다.
// node ID와 payload 형식, result 타입,
// 그리고 모든 CAN 하위 모듈이 재사용하는 메시지 컨테이너를 정의한다.
#ifndef CAN_TYPES_H
#define CAN_TYPES_H
...
// transport 계층이 사용하는 raw CAN frame 컨테이너다.
// 상위 계층은 보통 이것을 직접 다루지 않고,
// protocol 계층이 제공하는 논리적 CanMessage 객체를 사용한다.
typedef struct
{
    uint32_t id;
    uint8_t  dlc;
...
// protocol encoder는 이 더 풍부한 구조를 frame으로 바꾸고,
// 수신 경로에서 다시 복원한다.
typedef struct
{
    uint8_t  version;
...
// 추적 중인 CAN 요청이 끝났을 때 만들어지는 결과 기록이다.
// AppCore는 이 결과를 읽어,
// response와 timeout, 원격 동작 실패에 대한 operator 피드백을 만든다.
typedef struct
{
    uint8_t kind;
    uint8_t request_id;
