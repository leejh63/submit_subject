...
static void LinModule_HandleRxDoneEvent(LinModule *module)
{
    if (module == NULL)
    {
        return;
...
void LinModule_TaskFast(LinModule *module, uint32_t now_ms)
{
    ...
    {
        LinModule_HandleRxDoneEvent(module);
    }
...
void LinModule_TaskPoll(LinModule *module, uint32_t now_ms)
{
    ...
    if (module->ok_token_tx_pending != 0U)
    {
...
InfraStatus LinModule_RequestOk(LinModule *module)
{
    ...
    module->ok_token_tx_pending = 1U;
    module->tx_buffer[0] = module->config.ok_token;
    return INFRA_STATUS_OK;
}

// 현재 들고 있는 최신 status를 그대로 호출자에게 복사한다.
...
uint8_t LinModule_ConsumeSlaveOkToken(LinModule *module)
{
    ...
    pending = module->ok_token_rx_pending;
    module->ok_token_rx_pending = 0U;
    return pending;
}
