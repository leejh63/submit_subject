/*
 * portable LIN 프로토콜 상태기계 인터페이스다.
 * master와 slave 역할이 같은 API를 재사용하고,
 * runtime_io가 아래쪽에서 하드웨어 전용 callback 바인딩을 제공한다.
 */
#ifndef LIN_MODULE_H
#define LIN_MODULE_H
...
/*
 * portable LIN 모듈이 사용하는 하드웨어 바인딩 callback 표다.
 ...
 * 그리고 latch 상태를 slave2와 master policy 사이에 전달한다.
 */
typedef struct
{
    uint16_t adc_value;
...
/*
 * 하나의 모듈 인스턴스에 대한 정적 LIN 프로토콜 설정이다.
 * PID와 token 값, timing, 하드웨어 callback을 모두 묶어,
 * 상태기계가 시작되기 전에 한 번에 넘긴다.
 */
typedef struct
{
    uint8_t    role;
...
/*
 * master 또는 slave 모드에서 쓰는 전체 LIN 모듈 상태다.
 * protocol flag와 현재 buffer,
 * 그리고 application 계층과 공유하는 최신 상태 프레임을 함께 보관한다.
 */
typedef struct
{
    uint8_t       initialized;
