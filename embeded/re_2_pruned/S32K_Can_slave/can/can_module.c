/*
 * app 계층을 위한 CAN request 버퍼링 구현부다.
 * scheduler tick마다 CanService로 넘어가는 작업량을 조절하여,
 * 애플리케이션 코드가 transport보다 의도 표현에 집중하게 만든다.
 */
...
/*
 * app 수준 CAN 요청 하나를 소프트웨어 buffer에 올린다.
...
/*
 * queue에 쌓인 app 요청을 아래쪽 CAN service로 옮긴다.
 * tick당 제출량을 제한해 두어,
...
/*
 * app 계층에서 command 형태 CAN 요청 하나를 queue에 올린다.
 * 이 요청은 이후 task tick이 도착해,
 * 아래쪽 CAN service로 밀어 넣을 때까지 module queue에 머문다.
 */
...
/*
 * app 수준 command 처리 중 생성된 CAN response 하나를 queue에 올린다.
...
/*
 * 나중에 제출할 비동기 event 메시지 하나를 queue에 올린다.
 * module은 이 경로를 통해,
...
/*
 * CAN으로 보낼 짧은 text payload 하나를 queue에 올린다.
 * 이 경로는 UART 콘솔에서 나온,
