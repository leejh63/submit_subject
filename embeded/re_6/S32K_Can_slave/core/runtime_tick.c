...
// runtime 전체가 공통 시간 기준을 쓸 수 있게 된다.
...
// 세밀한 타이밍을 보고 싶을 때 millisecond 값과 별도로 참고할 수 있다.
...
// 실제 timer interrupt에서 시간을 올리고 등록된 hook를 호출한다.
// 해야 할 일은 짧게 유지하고,
// 무거운 정책 판단은 일반 task 쪽으로 넘기는 흐름을 전제로 둔다.
static void RuntimeTick_IrqHandler(void)
{
    uint32_t index;
...
// 참고:
// hook는 ISR 문맥에서 바로 실행된다.
// 따라서 여기에는 짧은 timeout service나 flag 기록 정도만 두고,
// 문자열 처리나 긴 로직은 계속 task 쪽에 유지하는 편이 안전하다.
