...
static void AppConsole_QueueLocalOk(AppConsole *console);
static void AppConsole_SendBleText(AppConsole *console, const char *text);
static void AppConsole_SendBleLine(AppConsole *console, const char *text);
static void AppConsole_SendBleStatusSnapshot(AppConsole *console);
static void AppConsole_HandleBleLine(AppConsole *console, const char *line);
static void AppConsole_ProcessBleUart(AppConsole *console, uint32_t now_ms);
static uint8_t AppConsole_HasDirtyOutput(const AppConsole *console);
static void AppConsole_ClearRenderDirtyFlags(AppConsole *console);
static InfraStatus AppConsole_SendPrimaryGuiSnapshot(AppConsole *console);
// 완성된 콘솔 입력 한 줄을 파싱한다.
// 일부 명령은 로컬에서 처리하고,
...
void AppConsole_SetResultText(AppConsole *console, const char *text)
{
    ...
    if (changed != 0U)
    {
        ...
        if ((console->initialized != 0U) && (console->ble_uart_enabled != 0U))
        {
            AppConsole_SendBleLine(console, text);
        }
    }
}
...
static void AppConsole_SendBleText(AppConsole *console, const char *text)
{
    if ((console == NULL) || (text == NULL) || (console->ble_uart_enabled == 0U))
    {
        return;
    }

    (void)UartService_RequestTx(&console->ble_uart, text);
}
...
static void AppConsole_SendBleLine(AppConsole *console, const char *text)
{
    ...
    AppConsole_SendBleText(console, buffer);
}
...
static uint8_t AppConsole_HasDirtyOutput(const AppConsole *console)
{
    ...
    return ((console->view.input_dirty != 0U) ||
            (console->view.task_dirty != 0U) ||
            (console->view.source_dirty != 0U) ||
            (console->view.result_dirty != 0U) ||
...
static InfraStatus AppConsole_SendPrimaryGuiSnapshot(AppConsole *console)
{
    ...
    AppConsole_ExtractTextLine(console->view.result_text, 0U, result_line, (uint16_t)sizeof(result_line));
    ...
    return UartService_RequestTx(&console->uart, render_buffer);
}
...
static void AppConsole_SendBleStatusSnapshot(AppConsole *console)
{
    ...
    AppConsole_ExtractTextLine(console->view.value_text, 0U, line0, (uint16_t)sizeof(line0));
    AppConsole_ExtractTextLine(console->view.value_text, 1U, line1, (uint16_t)sizeof(line1));
    AppConsole_ExtractTextLine(console->view.value_text, 2U, line2, (uint16_t)sizeof(line2));
    AppConsole_ExtractTextLine(console->view.result_text, 0U, result_line, (uint16_t)sizeof(result_line));

    if (line0[0] != '\0')
    {
        AppConsole_SendBleLine(console, line0);
    }

    if (line1[0] != '\0')
    {
        AppConsole_SendBleLine(console, line1);
    }

    if (line2[0] != '\0')
    {
        AppConsole_SendBleLine(console, line2);
    }

    (void)snprintf(buffer, sizeof(buffer), "Result : %s", result_line);
    AppConsole_SendBleLine(console, buffer);
}
...
static void AppConsole_HandleBleLine(AppConsole *console, const char *line)
{
    ...
    if (AppConsole_Tokenize(parse_buffer, argv, APP_CONSOLE_ARGV_CAPACITY, &argc) == 0U)
    {
        return;
    }

    if ((argc == 1U) && (strcmp(argv[0], "ok") == 0))
    {
        AppConsole_QueueLocalOk(console);
        return;
    }
    ...
    if ((argc == 1U) &&
        ((strcmp(argv[0], "status") == 0) || (strcmp(argv[0], "s") == 0)))
    {
        AppConsole_SendBleStatusSnapshot(console);
        return;
    }
    ...
    {
        AppConsole_SendBleLine(console, "ble commands: ok, status");
        return;
    }
}
...
static void AppConsole_ProcessBleUart(AppConsole *console, uint32_t now_ms)
{
    ...
    if ((console == NULL) || (console->ble_uart_enabled == 0U))
    {
        return;
    }

    UartService_ProcessRx(&console->ble_uart);
    if (UartService_HasError(&console->ble_uart) != 0U)
    {
        if (UartService_Recover(&console->ble_uart) != INFRA_STATUS_OK)
        {
            console->ble_uart_enabled = 0U;
            return;
        }
    }

    if (UartService_HasLine(&console->ble_uart) != 0U)
    {
        if (UartService_ReadLine(&console->ble_uart,
                                 line_buffer,
                                 (uint16_t)sizeof(line_buffer)) == INFRA_STATUS_OK)
        {
            AppConsole_HandleBleLine(console, line_buffer);
        }
    }

    UartService_ProcessTx(&console->ble_uart, now_ms);
}
...
InfraStatus AppConsole_Init(AppConsole *console, uint8_t node_id)
{
    ...
    if ((IsoSdk_UartIsSecondarySupported() != 0U) &&
        (UartService_InitWithInstance(&console->ble_uart,
                                      IsoSdk_UartGetSecondaryInstance()) == INFRA_STATUS_OK))
    {
        console->ble_uart_enabled = 1U;
    }
...
void AppConsole_Task(AppConsole *console, uint32_t now_ms)
{
    ...
    AppConsole_ProcessBleUart(console, now_ms);
...
void AppConsole_Render(AppConsole *console)
{
    ...
    if ((console->view.layout_drawn == 0U) ||
        (console->view.full_refresh_required != 0U) ||
        (AppConsole_HasDirtyOutput(console) != 0U))
    {
        if (AppConsole_SendPrimaryGuiSnapshot(console) == INFRA_STATUS_OK)
        {
            console->view.layout_drawn = 1U;
            console->view.full_refresh_required = 0U;
            AppConsole_ClearRenderDirtyFlags(console);
        }
    }
#endif
}
